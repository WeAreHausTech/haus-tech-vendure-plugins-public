...
Each plugin follows the same layout: `src/<name>.plugin.ts` (the `@VendurePlugin`), `src/index.ts` (public exports = package `main`/`types`), `project.json` (Nx targets), `package.json`, `README.mdx`, `CHANGELOG.md`, `tsconfig*.json`, `vitest.config.ts`.
