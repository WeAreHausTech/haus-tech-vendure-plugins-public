...
A Vendure plugin for importing and exporting product data via CSV. Enables bulk product management, data migration between environments, and integration with external systems.
...
export const config = {
  plugins: [
    ProductImportExportPlugin.init({
      ...
      exportOptions: {}, // Configure export options
    }),
  ],
}
```
