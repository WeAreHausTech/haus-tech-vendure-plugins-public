...
const SQLITE_SCHEMA_VERSION = '3.5.7'
const vendureVersion = createRequire(import.meta.url)('@vendure/core/package.json').version as string
const supportsSharedOptionGroups = vendureVersion.startsWith('3.6')
...
describe('ProductImportExportPlugin e2e', () => {
  ...
  it.skipIf(!supportsSharedOptionGroups)(
    'shares an option group across products that use the name:code syntax',
    async () => {
    const requestContextService = server.app.get(RequestContextService)
    const productImporter = server.app.get(ProductImporter)
    const productExportService = server.app.get(ProductExportService)
